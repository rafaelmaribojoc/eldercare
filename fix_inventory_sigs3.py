...
if form_ids:
    ...
    sigs = supabase.table('form_signatures').select('id, field_name, signer_name, signer_title').in_('form_submission_id', form_ids).eq('field_name', 'noted_by').execute()
    
    updated_count = 0
    for sig in sigs.data:
        name = (sig.get('signer_name') or '').upper()
        # The screenshot shows "JUAN HOMELIFE" signed as the Supervising HP.
        # And Candelaria Tingson is the Center Head.
        if 'JUAN HOMELIFE' in name:
            print(f"Updating signature ID {sig['id']} from 'noted_by' to 'submitted_by' for {name}")
